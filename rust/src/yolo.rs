...
use candle_nn::{
    batch_norm, conv2d, conv2d_no_bias, ops, BatchNorm, Conv2d, Conv2dConfig, VarBuilder,
};
use crate::Detection;
use wasm_bindgen::prelude::*;

#[wasm_bindgen]
pub fn safetensors_get_tag(weights: &[u8], key: &str) -> String {
    match safetensors::SafeTensors::deserialize(weights) {
        Ok(st) => {
            let dbg = format!("{:#?}", st);
            if let Some(pos) = dbg.find(key) {
                if let Some(start) = dbg[pos..].find('{') {
                    let sidx = pos + start;
                    if let Some(end_rel) = dbg[sidx..].find('}') {
                        let meta = &dbg[sidx..sidx + end_rel + 1];
...
impl YoloV8 {
    pub fn load(weights: &[u8]) -> Result<Self> {
        let dev = Device::Cpu;
        
        // Debug: Print keys
        if let Ok(st) = safetensors::SafeTensors::deserialize(weights) {
             let names = st.names();
             let msg = format!("Model keys (first 20): {:?}", names.iter().take(20).collect::<Vec<_>>());
             web_sys::console::log_1(&JsValue::from_str(&msg));
        }

        let vb = VarBuilder::from_slice_safetensors(weights, DType::F32, &dev)?;
    ...
    pub fn detect(&self, image_data: &[u8], width: u32, height: u32, conf_threshold: f32, iou_threshold: f32) -> Result<Vec<Detection>> {
        let dev = Device::Cpu;
        
        let img_buffer: image::ImageBuffer<image::Rgba<u8>, Vec<u8>> = 
            image::ImageBuffer::from_raw(width, height, image_data.to_vec())
            .ok_or_else(|| candle_core::Error::Msg("Failed to create image buffer from raw data".to_string()))?;
            
        let img = image::DynamicImage::ImageRgba8(img_buffer);
        let img = img.resize_exact(640, 640, image::imageops::FilterType::Triangle);
        let img = img.to_rgb8();
        let data = img.into_raw();
        let tensor = Tensor::from_vec(data, (640, 640, 3), &dev)?
            .permute((2, 0, 1))?
            .to_dtype(DType::F32)?
            .affine(1. / 255., 0.)?
            .unsqueeze(0)?;

        let (xs1, xs2, xs3) = self.net.forward(&tensor)?;
        let (xs1, xs2, xs3) = self.fpn.forward(&xs1, &xs2, &xs3)?;
        let pred = self.head.forward(&xs1, &xs2, &xs3)?.pred;
        
        let pred = pred.squeeze(0)?; // (4+nc, anchors)
        ...
        for index in 0..npreds {
            let p = Vec::<f32>::try_from(pred.i((.., index))?)?;
            
            let confidence = *p[4..].iter().max_by(|x, y| x.total_cmp(y)).unwrap();
            ...
            for i in 0..nclasses {
                if p[4 + i] > p[4 + class_index] {
                    class_index = i;
                }
            }
            
            if p[class_index + 4] > 0. {
                let cx = p[0];
                let cy = p[1];
                let w = p[2];
                let h = p[3];
        ...
        Ok(detections.into_iter().map(|b| Detection {
            bbox: [b.x1, b.y1, b.x2, b.y2],
            label: COCO_CLASSES.get(b.class_id).unwrap_or(&"Unknown").to_string(),
...
fn iou(b1: &Bbox, b2: &Bbox) -> f32 {
    ...
    intersection / (b1_area + b2_area - intersection)
}

fn non_max_suppression(bboxes: &[Bbox], threshold: f32) -> Vec<Bbox> {
    let mut bboxes = bboxes.to_vec();
    bboxes.sort_by(|a, b| b.confidence.partial_cmp(&a.confidence).unwrap());
    
    let mut selected = Vec::new();
    let mut active = vec![true; bboxes.len()];
    
    for i in 0..bboxes.len() {
        if !active[i] {
            continue;
        }
        selected.push(bboxes[i].clone());
        for j in (i + 1)..bboxes.len() {
            if active[j] && iou(&bboxes[i], &bboxes[j]) > threshold {
                active[j] = false;
            }
        }
    }
